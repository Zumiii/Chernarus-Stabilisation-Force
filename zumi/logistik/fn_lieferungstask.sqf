...
[
	format ["Auftrag%1.%2", tasknummer, _tsknr],
  localize "STR_ZOPS_LOGITASK_TITEL",
	format [localize "STR_ZOPS_LOGITASK_NOTIZ", _auftragsnummer],
	localize "STR_ZOPS_LOGITASK_TITEL",
	"created",
	(getMarkerpos "airportmarker"),
  "plane"
...
_handle = [
	{
		...
		_args params ["_tsk_obj","_tsknr"];
    if ({alive _x} count _tsk_obj == 0 || count ((_tsk_obj select {(_x getVariable ["verladen", -1]) == 0}) select {(_x inArea "Lagerzone")}) >= {alive _x} count _tsk_obj) then {
			if ({alive _x} count _tsk_obj == 0) then {
				[format ["Auftrag%1.%2", tasknummer, _tsknr], "failed"] call zumi_fnc_task_updaten;
			} else {
        [format ["Auftrag%1.%2", tasknummer, _tsknr], "succeeded"] call zumi_fnc_task_updaten;
      };
