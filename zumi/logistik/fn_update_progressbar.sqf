      ...
      for "_j" from 0 to (count _x) - 1 do {
        ...
        for "_k" from 0 to (count _liefereinheiten)-1 do {
          ...
          if ((_id isEqualTo _val) && (_kategorie isEqualTo _forEachindex)) then {
            _geaddet = true;
            _progressbar progressSetPosition ((progressPosition _progressbar) + (5*_platzverbrauch/100));
          };
        };
      };
    } forEach Bestellbare;
...
_progressbartext CtrlsetText format ["Used cargo space: %1/%2",(100*(progressPosition _progressbar)/50*100)/10,20];
