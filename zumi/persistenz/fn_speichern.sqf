...
////////////////////////////////////////*/
    ...
    if (count _spielerliste > 0) then {
      for "_i" from 0 to (count _spielerliste)-1 do {
        ...
        private _unitLoadout = getUnitLoadout [_spieler, false];
        ["write", ["Spielerliste",
          //Eintragsname
          ...
          [
            ...
            [
              ...
              _spieler getVariable ["323_panzer", 0],
              _spieler getVariable ["323_logistiker", 0],
            ...
            name _spieler,
            (_spieler getVariable ["BIS_fnc_setUnitInsignia_class", ""]),
            (_spieler getVariable ["vehicle", [-1, "", []]])
          ]
        ]] call _inidbi;
      };
    };
  ...
  //Rallypoint
    ...
    if ((count fortify_objekte_temp) > 0) then {
      {
          ...
          fortify_objekte pushBack [typeOf _obj, _posATL, _pby];
    ...
    if ((count lagerbestand_Temp) > 0) then {
      ...
      for "_i" from 0 to (count lagerbestand_Temp) - 1 do {
        ...
        if ((alive ((lagerbestand_Temp select _i) select 1)) && !(isNull ((lagerbestand_Temp select _i) select 1))) then {
