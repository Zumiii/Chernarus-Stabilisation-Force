...
Auftrags_Id = format ["%1", tasknummer + 1];
...
[
  ...
  localize "STR_ZOPS_TENSION_TITEL",
  format [localize "STR_ZOPS_TENSION_NOTIZ", ([_zeitansatz] call CBA_fnc_formatElapsedTime), [dayTime, "HH:MM"] call BIS_fnc_timeToString],
...
[
    ...
    if (cba_missionTime >= _zeitansatz) exitWith {
      ...
      [_handle] call CBA_fnc_removePerFrameHandler;
		...
		if (({((_x select 3) select 0) > 20} count villages < 1) || skip) exitWith {
			if (skip) then {
